...
public class AfkCheckTask implements Runnable {
	...
	public void run() {
		...
		if(!PopulationDensity.instance.perms.has(player, "populationdensity.idle") && kick && ConfigData.minimumPlayersOnlineForIdleBoot <= playersOnline) {
			PopulationDensity.instance.log.log("Kicked " + player.getName() + " for idling.");
		...
		//20L ~ 1 second		
		playerData.afkCheckTaskID = PopulationDensity.instance.getServer().getScheduler().scheduleSyncDelayedTask(PopulationDensity.instance, this, 20L * 60 * ConfigData.maxIdleMinutes);
	}
}
